...
@Component
@AllArgsConstructor
public class FoodStrategy implements MccStrategy {
    ...
    public TransactionResponseBody authorize(Transaction transaction) {
        ...
        if (accountBalance.getFoodBalance() >= amount) {
            accountBalance.setFoodBalance(accountBalance.getFoodBalance() - amount);
    ...
    public boolean hasAvailableBalance(Transaction transaction) {
        ...
        return accountBalance.getFoodBalance() >= amount;
    }

    @Override
    public List<String> getMccCode() {
        return List.of("5411", "5412");
    }
}
